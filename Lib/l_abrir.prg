...
Function AbrirDBF(AbrirDBF,AbrirCDX,AbrirALI,AbrirSHA,AbrirRUTA,AbrirORDEN)
   AbrirDBF:=UPPER(RTRIM(AbrirDBF))
   IF RIGHT(AbrirDBF,4)=".DBF"
      AbrirDBF:=LEFT(AbrirDBF,LEN(AbrirDBF)-4)
   ENDIF
...
***COMPROBAR EL INDICE***
   IF SELECT(AbrirALI)<>0
      SELECT(AbrirALI)
      IF AbrirCDX<>"SIN_INDICE" .AND. !UPPER(AbrirCDX+".CDX")==UPPER(DBORDERINFO(DBOI_FULLPATH))  .OR. ;
         AbrirCDX="SIN_INDICE" .AND. LEN(DBORDERINFO(DBOI_FULLPATH))>0
         &AbrirALI->( DBCLOSEAREA() )
      ENDIF
   ENDIF
...
**         DBUSEAREA( .T. ,  , AbrirDBF , AbrirALI , .F. , .F. )
      ...
      IF AbrirERROR1=0 .AND. AbrirERROR2=0
         EXIT
      ELSE
         IF MSGYESNO("Fichero: "+AbrirDBF+HB_OsNewLine()+ ;
                "Indice: "+AbrirCDX+HB_OsNewLine()+ ;
                "Alias: "+AbrirALI+HB_OsNewLine()+ ;
                "Compartido: "+IF(AbrirSHA="shared","Si","No")+HB_OsNewLine()+ ;
                "Ruta: "+AbrirRUTA+HB_OsNewLine()+ ;
                "Error DOS: "+LTRIM(STR(DOSERROR()))+HB_OsNewLine()+ ;
                "Error DBF: "+LTRIM(STR(AbrirERROR1))+"-"+Error_Nombre(AbrirERROR1)+HB_OsNewLine()+ ;
                "Error CDX: "+LTRIM(STR(AbrirERROR2))+"-"+Error_Nombre(AbrirERROR2)+HB_OsNewLine()+ ;
                "Llamado por: "+LTRIM(STR(PROCLINE(1)))+"-"+PROCNAME(1)+HB_OsNewLine()+HB_OsNewLine()+ ;
                "El fichero esta siento usado"+HB_OsNewLine()+ ;
...
Function AbrirDBF2(AbrirDBF,AbrirCDX,AbrirALI,AbrirSHA,AbrirRUTA)
   AbrirCDX:=IF(AbrirCDX=NIL,AbrirDBF,AbrirCDX)
   AbrirALI:=IF(AbrirALI=NIL,AbrirDBF,AbrirALI)
   AbrirSHA:=IF(AbrirSHA=NIL,"shared",AbrirSHA) //shared/Exclusive
