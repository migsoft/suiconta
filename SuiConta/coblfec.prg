...
procedure Coblfec()
   TituloImp:="Listado de cobros de facturas emitidas"
   ...
   DEFINE WINDOW W_Imp1 ;
      ...
      @130,110 COMBOBOX C_OrdLis WIDTH 150 ;
              ITEMS {'Fecha de cobro','Fecha vencimiento','Nombre cuenta'} ;
              VALUE 1
...
STATIC FUNCTION Coblfeci()
   ...
   FOR ANY2=YEAR(W_Imp1.D_Fec1.value) TO YEAR(W_Imp1.D_Fec2.value)
      ...
      RUTA2:=RUTA2[1]
      IF .NOT. FILE(RUTA2+"\FAC92.DBF") .OR. .NOT. FILE(RUTA2+"\COBROS.DBF")
         LOOP
      ENDIF
		AbrirDBF("COBROS",,,,RUTA2)
		GO TOP
		DO WHILE .NOT. EOF()
			IF FCOB>=W_Imp1.D_Fec1.value .AND. FCOB<=W_Imp1.D_Fec2.value
				IF AT(UPPER(W_Imp1.T_NomCob.value),UPPER(DESCRIP))<>0 .OR. LEN(RTRIM(W_Imp1.T_NomCob.value))=0
					aCobros1:={}
					AADD(aCobros1,NEMP)
					AADD(aCobros1,NFAC)
					AADD(aCobros1,SERFAC)
					AADD(aCobros1,FCOB)
					...
					SFAC2:=SERFAC
					NFAC2:=NFAC
					AbrirDBF("FAC92",,,,RUTA2,1)
					SEEK SERIE(SFAC2,NFAC2)
					IF CODCTA=VAL(W_Imp1.T_CodCta1.value) .OR. VAL(W_Imp1.T_CodCta1.value)=0
						AADD(aCobros1,FFAC)
						AADD(aCobros1,CODCTA)
						AADD(aCobros1,CLIENTE)
...
FOR N=1 TO LEN(aCobrosT)
   IF LIN>=190 .OR. PAG=0
      IF PAG<>0
         oprint:printdata(LIN,170,"Suma","times new roman",10,.F.,,"L",)
         oprint:printdata(LIN,210,MIL(TOT1,12,2),"times new roman",10,.F.,,"R",)
         LIN:=LIN+5
      ...
      LIN:=55
      oprint:printdata(LIN, 35,"Factura","times new roman",10,.F.,,"R",)
      ...
      oprint:printdata(LIN,125,"Fecha cobro","times new roman",10,.F.,,"L",)
      oprint:printdata(LIN,145,"Descripcion","times new roman",10,.F.,,"L",)
      oprint:printdata(LIN,210,"Importe","times new roman",10,.F.,,"R",)
      oprint:printdata(LIN,212,"Vencimiento","times new roman",10,.F.,,"L",)
      oprint:printdata(LIN,230,"Banco","times new roman",10,.F.,,"L",)
      oprint:printline(LIN+4,20,LIN+4,250,,0.5)
...
*/
   ...
   oprint:printdata(LIN, 35,LTRIM(STR(aCobrosT[N,2]))+"-"+aCobrosT[N,3],"times new roman",10,.F.,,"R",)
   ...
   oprint:printdata(LIN,125,DIA(aCobrosT[N,4],10),"times new roman",10,.F.,,"L",)
   oprint:printdata(LIN,145,aCobrosT[N,5],"times new roman",10,.F.,,"L",)
   oprint:printdata(LIN,210,MIL(aCobrosT[N,6],14,2),"times new roman",10,.F.,,"R",)
   oprint:printdata(LIN,212,DIA(aCobrosT[N,7],10),"times new roman",10,.F.,,"L",)
   oprint:printdata(LIN,230,aCobrosT[N,8],"times new roman",10,.F.,,"L",)
...
NEXT
   ...
   oprint:printdata(LIN,170,"Total","times new roman",10,.F.,,"L",)
   oprint:printdata(LIN,210,MIL(TOT1,12,2),"times new roman",10,.F.,,"R",)
/*
   SELEC FIN
   FIN->( DBCLOSEAREA() )
